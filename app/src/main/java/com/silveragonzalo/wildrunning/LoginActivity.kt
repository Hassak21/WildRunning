...
class LoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ui.lyTerms.visibility = View.INVISIBLE
        etEmail = ui.etEmail
        ...
        mAuth.signInWithEmailAndPassword(email, password)
            .addOnCompleteListener(this) { task ->
                if (task.isSuccessful) {
                    ...
                    if (ui.lyTerms.visibility == View.INVISIBLE)
                        ui.lyTerms.visibility = View.VISIBLE
                    else {
                        var check = ui.cbAcept
                        if (check.isChecked)
                            registrar()
                    }
            ...
            mAuth.sendPasswordResetEmail(e)
                .addOnCompleteListener { task ->
                    if (task.isSuccessful)
                        ...
                        Toast.makeText(this,"No se encuentra el usuario con el correo $e", Toast.LENGTH_SHORT).show()
                }
        }
        else {
